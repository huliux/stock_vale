...
class PresentValueCalculator:
    ...
    def calculate_present_values(self,
                                 ...
                                 wacc: float) -> Tuple[Optional[float], Optional[float], Optional[str]]:
        """
        ...
        try:
            ...
            if forecast_df_copy['pv_ufcf'].apply(lambda x: isinstance(x, Decimal) and (x.is_nan() or x.is_infinite())).any():
                ...
                return None, None, error_msg # Return None for both PVs if any intermediate PV is invalid
            # --- End check ---
            ...
            if pv_terminal_value_decimal.is_nan() or pv_terminal_value_decimal.is_infinite():
                 ...
                 # 可能 UFCF 现值仍然有效
                 return float(pv_ufcf_decimal), None, error_msg

            # Convert results back to float
            return float(pv_ufcf_decimal), float(pv_terminal_value_decimal), None # 成功
