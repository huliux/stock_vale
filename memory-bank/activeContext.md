...
11. **(新功能规划) 估值结果缓存与历史记录管理:**
    *   **目标:** 允许用户保存单次估值的核心参数和结果，方便后续查看、对比和管理。
    *   **初期实现 (V2方案):**
        *   **存储:** 每个估值记录保存为一个独立的JSON文件，存放于 `valuation_history/` 目录。
        *   **UI:**
            *   “保存本次估值”按钮及备注输入。
            *   新增“历史估值”标签页，列表展示历史记录（股票代码、保存时间、备注、核心估值、关键假设如营业利润率模式、SGA&RD占收比模式、WACC值）。
            *   操作：查看完整历史估值详情（只读快照）、删除记录。
        *   **不包含（远期）：** 估值对比功能。
12. **(重大决策) 前端技术栈与架构演进:**
    *   **背景:** 现有Streamlit前端在灵活性和复杂交互定制方面存在局限，难以满足项目长远发展需求。
    *   **决策:**
        *   **前端框架选型:** 采用 **Vue.js (配合Nuxt.js或Vite)** 作为新的前端技术栈。
        *   **项目结构:** 迁移到**轻量级Monorepo结构**，在当前Git仓库内通过目录划分管理后端、新前端及共享代码。
    *   **目标:** 提升应用的扩展性、可靠性、UI定制能力，优化开发体验和代码可维护性。
...
-   **Monorepo 结构搭建与后端准备 (已完成):**
    *   在 `dev/stock_rs-project-restructure` 分支上创建了 `packages/` 目录。
    *   后端 FastAPI 代码已迁移至 `packages/fastapi-backend/`。
    *   使用 `uv` 和 Python 3.10 整顿了后端Python环境，包括创建 `pyproject.toml`，安装依赖生成 `uv.lock`，并更新了 Dockerfile。
    *   后端服务已通过 `uvicorn` 命令验证可在新结构和环境下成功启动。
-   **Vue.js 前端骨架搭建 (已完成):**
    *   在 `packages/vue-frontend/` 初始化了 Vue.js (Vite, TypeScript, Pinia, Vue Router) 项目。
    *   创建了共享类型包 `packages/shared-types/` 并定义了DCF估值相关的API类型。
    *   搭建了前端应用的基础布局 (`App.vue`) 和路由 (`router/index.ts`)，包含“DCF估值”和“股票筛选器”页面。
    *   为DCF估值和股票筛选器功能创建了主要的Vue组件骨架，并集成了模拟数据和事件处理。
    *   创建了API客户端模块 (`apiClient.ts`) 的骨架。
    *   解决了前端TypeScript配置问题及Vue编译器宏相关的警告。
    *   前端开发服务器 (`npm run dev`) 可正常运行，应用基本框架可见。
...
**A. 前端功能实现与API对接 (Vue.js - `dev/stock_rs-project-restructure` 分支)**
1.  **后端API准备 (并行进行):**
    *   [ ] **股票筛选器API:** 在 `packages/fastapi-backend/` 中为股票筛选器功能创建新的FastAPI端点。
        *   该端点应能处理前端发送的筛选条件。
        *   封装 `stock_screener_data.py` 中的逻辑，包括调用Tushare API获取数据、数据缓存、数据合并与筛选。
        *   定义清晰的API请求和响应模型 (可参考或使用 `packages/shared-types/` 中的定义)。
    *   [ ] **DCF估值API适配:** 检查并确保现有的 `/api/v1/valuation` 端点：
        *   其请求体与 `packages/shared-types/src/index.ts` 中定义的 `ApiDcfValuationRequest` 兼容。
        *   其响应体与 `ApiDcfValuationResponse` 兼容。
        *   处理好百分比到小数的转换（如果前端发送的是用户友好百分比）。
2.  **前端API客户端完善:**
    *   [ ] 在 `packages/vue-frontend/src/services/apiClient.ts` 中，根据后端API的实际情况，添加或完善具体的API调用函数 (例如 `fetchStockScreenerResults`, `performDcfValuation`)。
    *   [ ] 考虑为不同的业务模块创建专门的service文件 (例如 `stockScreenerService.ts`, `valuationService.ts`) 来组织API调用。
3.  **股票筛选器功能实现 (Vue.js):**
    *   [ ] 在 `StockScreenerView.vue` 和其子组件中，替换模拟数据逻辑为通过 `apiClient` 调用后端股票筛选器API。
    *   [ ] 实现加载状态和错误处理。
    *   [ ] 实现数据更新按钮的真实API调用。
    *   [ ] 完善筛选结果表格的展示和交互。
4.  **DCF估值功能实现 (Vue.js):**
    *   [ ] 在 `DcfValuationView.vue` 和其子组件中，替换模拟API调用为通过 `apiClient` 调用后端DCF估值API。
    *   [ ] 实现加载状态和错误处理。
    *   [ ] 确保从筛选器传递过来的股票代码能正确用于估值请求。
    *   [ ] 完善DCF参数输入的校验和用户体验。
    *   [ ] 完善估值结果（包括基本信息、核心指标、详细预测、敏感性分析、LLM总结、数据警告）的展示。
        *   对于表格数据（详细预测、敏感性分析），考虑使用更专业的表格组件或自定义渲染逻辑以替代当前的 `<pre>` 标签。
5.  **UI/UX优化与测试:**
    *   [ ] 对已实现的前端页面进行全面的UI/UX审查和优化。
    *   [ ] 进行组件级和页面级的功能测试。
    *   [ ] (可选) 编写前端单元测试和集成测试 (Vitest)。

**B. 其他待办任务 (并行或后续考虑)**
6.  **实现估值结果缓存与历史记录管理功能 (V2方案):** (优先级待前端MVP稳定后再评估)
    *   [ ] 定义JSON文件结构。
    *   [ ] 实现保存估值功能。
    *   [ ] 实现历史记录列表展示。
    *   [ ] 实现查看历史详情功能。
    *   [ ] 实现删除历史记录功能。
    *   [ ] 功能测试和UI/UX测试。
7.  **处理 Pytest 警告:** (可选, 优先级较低) 解决 `pytest` 输出中的 `FutureWarning`。
8.  **确认用户满意度:** 在新前端功能稳定后, 进一步确认UI细节和整体体验。
9.  **上下文窗口管理:** 在主要功能模块完成后, 或上下文窗口使用率显著增高时, 应使用 `new_task` 工具创建新任务。
10. **规则文件更新 (`.clinerules/`):** (可选, 定期进行) 根据项目经验检查并更新。
