...
- **新功能：股票筛选器开发 (进行中)**
    ...
    - [ ] **阶段四：测试与文档 (整合后)**
        ...
        - [ ] 用户界面和体验测试。
        - [ ] (可选) 补充必要的单元测试。
        - [x] 更新所有相关的 `memory-bank/` 文档以反映筛选器功能的最终实现和整合状态 (已完成)。

- **前端重构与Monorepo搭建 (Vue.js - 新计划 - 分支: `dev/stock_rs-project-restructure`)**
    - [x] **阶段 0: 准备工作**
        - [x] 创建新的Git分支 (`dev/stock_rs-project-restructure`) - 用户已完成。
        - [x] 更新 `memory-bank` (activeContext.md, progress.md, techContext.md) 以反映新决策和计划。
    - [x] **阶段 1: Monorepo 目录结构搭建与代码迁移 (初步完成)**
        - [x] 创建 `packages/` 目录。
        - [x] 迁移现有后端代码到 `packages/fastapi-backend/`。 (后续需调整导入路径)
        - [x] 在 `packages/` 下初始化新的Vue.js项目 `vue-frontend` (Vite, TypeScript, Pinia, Vue Router) 并安装依赖。
        - [x] (可选但推荐) 创建共享类型包 `packages/shared-types/` (含package.json, tsconfig.json, src/index.ts)。
        - [x] 更新根目录 `.gitignore`。
        - [x] 创建/配置VSCode工作区 `.vscode/settings.json`。
        - [x] (推荐) 创建根目录 `docker-compose.yml`。
    - [x] **Python后端环境整顿 (uv + Python 3.10) (已完成)**
        - [x] 用户通过Homebrew安装uv。
        - [x] 清理 `packages/fastapi-backend/` 下旧的 `.venv`。
        - [x] 在 `packages/fastapi-backend/` 创建 `pyproject.toml` 并定义依赖。
        - [x] 使用 `uv venv .venv --python 3.10` 创建新虚拟环境。
        - [x] 用户激活虚拟环境后，使用 `uv sync` 安装依赖并生成 `uv.lock`。
        - [x] 更新 `packages/fastapi-backend/Dockerfile` 使用Python 3.10和uv。
        - [x] 更新 `.vscode/settings.json` 指向新Python环境。
    - [x] **阶段 1.1: 后端代码路径调整与验证** (已完成)
        - [x] 检查 `packages/fastapi-backend/` 内部Python导入路径 (确认为兼容)。
        - [x] 验证后端服务能在新的Monorepo结构和uv环境下通过 `uvicorn` 命令成功启动 (无导入错误)。 (Docker验证推后)
    - [x] **阶段 2: 前端MVP功能开发 (Vue.js) (骨架完成)**
        - [x] 搭建基础布局和路由 (DCF估值、股票筛选器标签页)。
        - [x] 实现API客户端模块 (骨架完成)。
        - [x] 迁移/实现股票筛选器功能 (组件骨架和视图集成完成)。
        - [x] 迁移/实现DCF估值页面功能 (组件骨架和视图集成完成)。
        - [x] 定义共享类型 (shared-types) 用于DCF估值。
        - [x] 解决前端TypeScript配置及编译器宏警告。
    - [ ] **阶段 2.1: 前端功能实现与API对接**
        - [ ] (后端) 为股票筛选器创建FastAPI数据接口。
        - [ ] (后端) 确保DCF估值API与共享类型兼容。
        - [ ] (前端) 将股票筛选器连接到后端API。
        - [ ] (前端) 将DCF估值功能连接到后端API。
        - [ ] (前端) UI细节完善和交互增强。
    - [ ] **阶段 3: 逐步完善与迭代**
...
- **新功能：估值结果缓存与历史记录管理 (V2方案 - 待开始)** (优先级待前端重构有一定进展后再评估)
    - [ ] **阶段一：核心功能MVP实现 (JSON文件存储)**
        - [ ] **1.1 定义JSON文件结构:** 明确保存字段和层级。
        - [ ] **1.2 实现保存估值功能:**
            - [ ] 添加“保存估值”按钮和备注输入UI。
            - [ ] 实现数据收集与JSON写入逻辑。
        - [ ] **1.3 实现历史记录列表展示:**
