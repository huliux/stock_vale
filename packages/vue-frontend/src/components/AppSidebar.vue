...
import DcfParametersForm from '@/components/DcfParametersForm.vue' // Added import
import {
  Sidebar,
  SidebarContent,
  SidebarFooter,
...
<template>
  <Sidebar class="overflow-hidden *:data-[sidebar=sidebar]:flex-row" v-bind="props">
    ...
    <Sidebar collapsible="none" class="hidden flex-1 md:flex">
      <SidebarHeader v-if="navItems.find(item => item.id === activeNavItemId)" class="border-b p-4">
        <h3 class="text-lg font-semibold">{{navItems.find(item => item.id === activeNavItemId)?.title}}</h3>
      </SidebarHeader>
      <SidebarContent class="p-2">
        <div v-if="activeNavItemId === 'valuation'" class="h-full">
          <DcfParametersForm :is-loading="props.isLoading" :initial-stock-code="props.initialStockCode"
            @submit-valuation="payload => emit('submit-valuation', payload)"
            @validation-error="message => emit('validation-error', message)" class="h-full overflow-y-auto" />
        </div>
